  ...
  return (
    <div>
      {splashActive ? (
        ...
        <div className="relative m-0 p-0 h-screen w-screen">
          <img src={LogImage} alt="Login" className="absolute top-0 left-0 w-full h-full object-cover" />
          <div className="p-10 bg-whiteColor absolute top-1/4 left-1/2 transform -translate-x-1/2 w-[90%] sm:w-[60%] lg:w-[40%] rounded-md text-center shadow-lg">
            <Link to='/'>
              <span className='logo text-secondaryColor font-bold text-2xl flex justify-center items-center max-sm:text-xl mb-2'>
                Fresh <span className='text-secondaryColor flex'>F <FaSoap className="text-secondaryColor" size={30} /> ld</span>
              </span>
            </Link>
            <h1 className="text-headerTextColor font-semibold text-2xl mb-5">Welcome Back, <span className='text-secondaryColor font-bold'>Login</span></h1>
            <form onSubmit={handleSubmit} className='flex flex-col items-center justify-center gap-4'>
              ...
              <input
                ...
                placeholder='Email'
                className='p-2 w-full rounded-md outline-0 border bg-transparent border-mainColor focus:border-secondaryColor'
              />
              <input
                type="password"
                name="password"
                ...
                className='p-2 w-full rounded-md outline-0 border bg-transparent border-mainColor focus:border-secondaryColor'
              />
              <button type="submit" className='btnbtn p-3 rounded-md border bg-textColor text-whiteColor w-full text-xl hover:bg-secondaryColor hover:text-whiteColor'>
                Sign In
              </button>
            </form>
            <h1 className='mt-5 text-xl'>
              Don't Have an Account? - <Link to="/register" className='text-secondaryColor'>Register</Link>
            </h1>
          </div>
        </div>
      )}
